const requestsContainer = document.getElementById('requests-container');
        ...
        function createRequestElement(data = {}) {
            ...
            executeButton.addEventListener('click', () => executeRequest(requestElement));
            ...
            methodSelect.addEventListener('change', () => updateRequestColor(requestElement));
        ...
        function executeRequest(requestElement) {
            const method = requestElement.querySelector('.method-select').value;
        ...
        function updateSidebar() {
            ...
            requestsContainer.querySelectorAll('.request-card').forEach(requestElement => {
                ...
                const requestName = requestElement.querySelector('.request-name').value || 'Unnamed Request';
                const listItem = document.createElement('li');
                listItem.innerHTML = `<a href="#${requestId}" class="block p-2 hover:bg-gray-100 rounded">${requestName}</a>`;
        ...
        function saveSession() {
            ...
            if (sessionIndex !== -1) {
                sessions[sessionIndex] = { name: sessionName, requests };
            } else {
                if (sessions.length >= 5) {
                    sessions.shift();
                }
                sessions.push({ name: sessionName, requests });
            }
